...
import { ExampleConfigurator } from "./ExampleConfigurator.js";
...
const randomThreatLevel = () =>
  random(0, 20) ? null : sample(["Low", "Medium", "High"]);
...
class CityGrid extends React.Component {
  ...
  _generateData() {
    ...
    for (let zone = 0; zone < 20; zone++) {
      ...
      for (let host = 0; host < hostCount; host++) {
        zoneData.children.push({
          ...
          ip: `${random(1, 255)}.${random(1, 255)}.${random(1, 255)}.${random(
            1,
            255
          )}`,
        });
    ...
    let layout =
      this._treemapLayout ||
      (this._treemapLayout = treemap().padding(2).tile(treemapResquarify));
    ...
    console.log(
      `Generated ${zoneHierarchy.children.length} zones with ${
        zoneHierarchy.leaves().length
      } hosts`
    );
  }
  ...
  _onMouseWheel(e) {
    ...
    if (shiftKey) {
      this.setState(
        {
          cameraElevation: Math.max(
            1,
            this.state.cameraElevation + deltaY / 10
          ),
        },
        after
      );
      ...
      this.setState(
        { cameraDistance: Math.max(1, this.state.cameraDistance + deltaY / 5) },
        after
      );
    }
  }
  ...
  render() {
    ...
    return (
      <div
        className="w-[100%] h-[100vh] bg-[#ccc]"
        ...
        {true ? (
          <Canvas3D
            ...
            camera={{
              ...
              up: CAMERA_UP,
              transition:
                state.enableTransitions && !this._isWheeling
                  ? {
            ...
            objects={[
                ...
                children: [
                  ...
                  },
                  {
                    ...
                    template: {
                      ...
                      x: (host) => host.x0,
                      y: (host) => host.y0,
                      z: state.selectedHostIp
                        ? (host) =>
                            host.data.ip === state.selectedHostIp
                              ? 0
                              : DROPAWAY_Z
                        : 0,
                      rotateZ: 0,
                      ...
                      transition: state.enableTransitions
                        ...
                        : null,
                      animation: (host) =>
                        host.data.ip === state.hoveredHostIp
                          ? [
                              {
                                0: { rotateZ: 0 },
              ...
              },
              {
                ...
                items: [
                  {
                    type: "button",
                    onClick: this._changeHeights,
                    label: "Change Heights",
                  },
                  {
                    type: "button",
                    onClick: this._changeThreatLevels,
                    label: "Change Threats",
                  },
                  {
                    type: "button",
                  ...
                  {
                    type: "button",
                    onClick: this._gotoRandomCameraPos,
                    label: "Random Camera Pos",
                  },
